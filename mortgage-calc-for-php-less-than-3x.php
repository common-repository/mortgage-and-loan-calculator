...
for ($j = 0; $j < $months; $j++) {
  ...
  $startDateTemp->modify('+28 days');
  $startDate = new DateTime($startDateTemp->format('Y').'-'.$startDateTemp->format('m').'-'.$startDateTemp->format('t'));
  $startDate = $startDate->format($formatYmd);
  if ($loan == 0) break;
}
$tmortages = $mortgages;
...
if (isset($_POST['monthly_mortgage_calc_widget'])) {
  ...
  $response = "
					<table class='style1' cellspacing='0'>
						<tr >
							<td>Changes paid off date to:</td>
							<td><input id='advanced_mortgage_calc_finish' name='advanced_mortgage_calc_finish' readonly='readonly' type='text' value='" . $mortgages[$total_rates - 1]['month'] . "'/></td>
						</tr>
						<tr >
							<td>Monthly Payments: $</td>
							<td><input id='advanced_mortgage_calc_monthly_mortgate' name='advanced_mortgage_calc_monthly_mortgate' readonly='readonly' type='text' value='$original_mortgage'/></td>
						</tr>
					</table>
					<table class='style1' cellspacing='0'>
						<tr  class='first' ><td><strong>Month / Year</strong></td> <td><strong>Payment</strong></td> <td><strong>Principal Paid</strong></td> <td><strong>Interest Paid</strong></td> <td><strong>Total Interest</strong></td> <td><strong>Balance</strong></td> </tr>
						" . $message_post . "
					</table>";
  echo $response;
}
